...
import { Monitor, Search, PlayCircle, Key, CheckCircle2, RefreshCw, Filter, UserX, ArrowDownAZ, ArrowUpZA, AlertTriangle } from 'lucide-react';
...
const StatusTesTab = ({ currentUser, students, refreshData }: { currentUser: User, students: any[], refreshData: () => void }) => {
    ...
    const [confirmModal, setConfirmModal] = useState<{isOpen: boolean, username: string}>({
        isOpen: false,
        username: ''
    });
    ...
    const handleReset = async (username: string) => { 
        setConfirmModal({ isOpen: true, username });
    };

    const executeReset = async () => {
        const username = confirmModal.username;
        setResetting(username); 
        try { 
            await api.resetLogin(username); 
            showToast(`Login ${username} berhasil direset.`, "success");
            refreshData(); 
        } catch(e) { 
            console.error(e); 
        ...
        <div className="space-y-6 fade-in">
            ...
            <div className="bg-white rounded-[1.5rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
                ...
                <ConfirmationModal 
                    ...
                    title="Konfirmasi Reset Login"
                    message={`Apakah Anda yakin ingin mereset login untuk ${confirmModal.username}?\n\nSiswa akan logout otomatis dan status menjadi OFFLINE.`}
